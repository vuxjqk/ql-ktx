        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 grid gap-6">
            ...
            <div class="flex items-center justify-between mx-6">
                <div>
                    ...
                    <p class="text-sm text-gray-600 mt-1">{{ __('Thêm phòng mới vào hệ thống') }}</p>
                </div>
                ...
                <form action="{{ route('rooms.store') }}" method="post" enctype="multipart/form-data"
                    ...
                    <div>
                        ...
                        <x-text-input id="room_code" class="block mt-1 w-full" type="text" name="room_code"
                            :value="old('room_code')" required autofocus autocomplete="room_code" :placeholder="__('Nhập mã phòng')" />
                            ...
                            @foreach ($options as $branchName => $floors)
                                <optgroup label="{{ $branchName }}">
                                    @foreach ($floors as $floorId => $floorName)
                                        <option value="{{ $floorId }}" @selected(old('floor_id'))>
                                            {{ $floorName }}
                                        </option>
                                    @endforeach
                                </optgroup>
                    ...
                    <div>
                        ...
                        <x-text-input id="price_per_day" class="block mt-1 w-full" type="number" name="price_per_day"
                            :value="old('price_per_day')" required autocomplete="off" :placeholder="__('Nhập giá theo ngày')" />
                    ...
                    <div>
                        ...
                        <x-text-input id="price_per_month" class="block mt-1 w-full" type="number"
                            name="price_per_month" :value="old('price_per_month')" required autocomplete="off" :placeholder="__('Nhập giá theo tháng')" />
                    ...
                    <div>
                        ...
                        <x-text-input id="capacity" class="block mt-1 w-full" type="number" name="capacity"
                            :value="old('capacity')" required autocomplete="capacity" :placeholder="__('Nhập sức chứa')" />
                    ...
                    <div>
                        ...
                        <x-text-input id="current_occupancy" class="block mt-1 w-full" type="number"
                            name="current_occupancy" :value="old('current_occupancy', 0)" required autocomplete="current_occupancy"
                    ...
                    <div>
                        ...
                        <x-select id="is_active" class="block mt-1 w-full" :options="[
                            ...
                            :selected="old('is_active', '1')" required :placeholder="__('Chọn trạng thái')" />
                            ...
                            @foreach ($services as $id => $name)
                                <label for="service_{{ $id }}" class="inline-flex items-center">
                                    <input id="service_{{ $id }}" type="checkbox"
                                        ...
                                        name="services[]" value="{{ $id }}" @checked(in_array($id, old('services', [])))>
