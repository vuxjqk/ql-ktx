        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 grid gap-6">
            ...
            <div class="flex items-center justify-between mx-6">
                <div>
                    ...
                    <p class="text-sm text-gray-600 mt-1">{{ __('Cập nhật thông tin nhân viên trong hệ thống') }}</p>
                </div>
                ...
                <form action="{{ route('users.update', $user) }}" method="post" enctype="multipart/form-data"
                    ...
                    <div>
                        ...
                        <x-select id="role" class="block mt-1 w-full" :options="[
                            ...
                            :selected="old('role', $user->role)" required :placeholder="__('Chọn vai trò')" />
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <form action="{{ route('users.updateBranches', $user) }}" method="post"
                    class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                    @csrf
                    @method('put')
                            ...
                            @foreach ($branches as $id => $name)
                                <label for="branch_{{ $id }}" class="inline-flex items-center">
                                    <input id="branch_{{ $id }}" type="checkbox"
                                        ...
                                        name="branches[]" value="{{ $id }}" @checked(in_array($id, old('branches', $user->branches->pluck('id')->toArray())))>
